...
#         if not user_doc:
#             self.db.create_document('whatsapp_conversations', normalized_phone, {'state': 'initial', 'interaction_count': 1})
#             await self.send_welcome_message(from_number, False)
#             return "New user welcome message sent"

#         if body.strip().lower() == "exit":
#             self.db.update_document('whatsapp_conversations', normalized_phone, {'state': 'initial', 'interaction_count': interaction_count + 1})
#             send_message(from_number, "You have exited. Type anything to start again.")
#             return "Exit acknowledged and state reset"

#         if state == 'initial':
#             await self.send_welcome_message(from_number, True)
...
#         welcome_message = "Welcome back to AOI! It's great to see you again. If you're ready to continue, type 'yes'." if is_returning_user else "Welcome to the AOI (AI Objectives Institute)! If you are ready to answer some short questions for me to get to know you, type 'yes'."
...
#         if not user_doc or body.strip().lower() == "exit":
#             self.db.create_document('whatsapp_conversations', normalized_phone, {
#                 'state': 'initial', 
#                 'interaction_count': interaction_count + (0 if not user_doc else 1)
#             })
#             send_message(from_number, "You have exited. Type anything to start again.")
#             self.welcome_message_sent[normalized_phone] = False
#             return "Exit acknowledged and state reset"
